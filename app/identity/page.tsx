...
import { getStrandConfig } from '@/lib/strand-config';
...
function ProviderIcon({ provider, className }: { provider: string; className?: string }) {
  return <StrandIcon icon={provider} className={className} />;
}
...
function StrandIcon({ icon, className }: { icon: string; className?: string }) {
...
function StrandsSection({ strands, onRefresh, configuredProviders }: {
  ...
  const linkedProviders = new Set(strands.filter(s => !s.strandType || s.strandType === 'oauth').map(s => s.provider));
    ...
    <div className="mt-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="text-[9px] text-zinc-400 dark:text-zinc-600 uppercase tracking-widest mb-1">
            Identity Strands // $401 Proof Chain
          </div>
          <div className="flex items-center gap-3">
            <h3 className="text-lg font-mono font-bold text-zinc-900 dark:text-white uppercase tracking-tight">
              DNA Strands
            </h3>
            <span className={`text-[10px] font-mono font-bold uppercase tracking-widest ${strengthColors[strength.level]}`}>
              {strands.length} linked &middot; Lv.{strength.levelNumber} {strength.label}
            </span>
          </div>
        </div>
      </div>
      ...
      {strands.length > 0 && (
        <div className="space-y-2">
          {strands.map((strand) => {
            const config = getStrandConfig(strand.strandType || 'oauth', strand.strandSubtype, strand.provider);
            return (
              <div
                key={strand.id}
              ...
              >
                <div className={`shrink-0 ${config.color}`}>
                  <StrandIcon icon={config.icon} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-mono font-bold text-zinc-900 dark:text-white">
                      {strand.label || config.label}
                    </span>
                    <StatusBadge status={strand.broadcastStatus} />
                    {strand.source && strand.source !== 'path401' && (
                      <span className="text-[8px] font-mono uppercase tracking-widest px-1.5 py-0.5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">
                        via {strand.source}
                      </span>
                    )}
                  </div>
                  <div className="text-[10px] font-mono text-zinc-500 mt-0.5">
                    {strand.providerHandle && `@${strand.providerHandle}`}
                    ...
                    {!strand.providerHandle && config.description}
                  </div>
                  <div className="text-[9px] font-mono text-zinc-400 dark:text-zinc-600 mt-0.5">
                    {new Date(strand.oauthVerifiedAt || strand.createdAt).toLocaleDateString()}
                    <span className="text-zinc-300 dark:text-zinc-700"> &middot; +{config.pointValue}pts</span>
                    {strand.strandTxid && (
                      <>
                        {' '}&middot;{' '}
                        <a
      ...
      {/* Advanced Strands (available via bit-sign) */}
      <div className="mt-4">
        <div className="text-[9px] text-zinc-400 dark:text-zinc-600 uppercase tracking-widest mb-2">
          Advanced Verification // via bit-sign.online
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {[
            { label: 'Self-Attest', icon: 'pen' as const, level: 'Lv.2' },
            { label: 'ID Document', icon: 'file' as const, level: 'Lv.2' },
            { label: 'Peer Attest', icon: 'users' as const, level: 'Lv.3' },
            { label: 'KYC', icon: 'fingerprint' as const, level: 'Lv.4' },
          ].map((item) => (
            <a
              key={item.label}
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 p-3 border border-zinc-100 dark:border-zinc-900 bg-zinc-50 dark:bg-zinc-950 font-mono text-xs text-zinc-500 uppercase tracking-widest hover:border-zinc-300 dark:hover:border-zinc-700 transition-all"
            >
              <StrandIcon icon={item.icon} className="w-4 h-4" />
              <span>{item.label}</span>
              <span className="text-[8px] text-zinc-400 dark:text-zinc-600 ml-auto">{item.level}</span>
            </a>
          ))}
        </div>
      </div>
